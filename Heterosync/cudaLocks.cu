                          ...
                          const int NUM_SM)
{
  ...
  cudaMalloc(&cpuLockData->semaphoreBuffers, sizeof(unsigned int) * 5 * cpuLockData->semaphoreCount);
  ...
  */
  ...
  cudaMemset(cpuLockData->semaphoreBuffers, 0, sizeof(unsigned int) * cpuLockData->semaphoreCount * 5);
