...
int main(int argc, char* argv[])
{
    ...
    unsigned int* d_in;
    checkHipErrors(hipMalloc(&d_in, sizeof(unsigned int) * h_in_len));
    checkHipErrors(hipMemcpy(d_in, h_in, sizeof(unsigned int) * h_in_len, hipMemcpyHostToDevice));

    // Set up device-side memory for output
    unsigned int* d_out_blelloch;
    checkHipErrors(hipMalloc(&d_out_blelloch, sizeof(unsigned int) * h_in_len));
    ...
    checkHipErrors(hipMemcpy(h_out_blelloch, d_out_blelloch, sizeof(unsigned int) * h_in_len, hipMemcpyDeviceToHost));
    checkHipErrors(hipFree(d_out_blelloch));
    checkHipErrors(hipFree(d_in));
