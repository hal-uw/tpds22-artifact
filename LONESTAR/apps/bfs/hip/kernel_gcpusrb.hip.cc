...
{
  short t;
  // ** TODO: If this needs to be used on AMD hardware, assembly code
  // needs to be written to use the GLC bit to go to the L2
  // NVIDIA ASM: asm ("ld.global.cg.u8 %0, [%1];" : "=h"(t) : "r"(addr));
  t = *addr;
  return (bool)t;
}
...
#define TB_SIZE 32
const char *GGC_OPTIONS = "coop_conv=False $ outline_iterate_gb=True $ backoff_blocking_factor=4 $ parcomb=True $ np_schedulers=set(['fg', 'tb', 'wp']) $ cc_disable=set([]) $ tb_lb=True $ hacks=set([]) $ np_factor=8 $ instrument=set([]) $ unroll=[] $ read_props=None $ outline_iterate=True $ ignore_nested_errors=False $ np=True $ write_props=None $ quiet_cgen=True $ retry_backoff=True $ hip.graph_type=basic $ hip.use_worklist_slots=True $ hip.worklist_type=basic";
      ...
      if (enable_lb)
        {
          ...
          hipLaunchKernelGGL(Inspect_bfs_kernel_dev, dim3(blocks), dim3(__tb_bfs_kernel), 0, 0, gg, LEVEL, t_work.thread_work_wl, t_work.thread_src_wl, enable_lb, pipe.in_wl(), pipe.out_wl());
          ...
          if (num_items != 0)
            {
              ...
              hipDeviceSynchronize();
              hipLaunchKernelGGL(bfs_kernel_dev_TB_LB, dim3(blocks), dim3(__tb_bfs_kernel), 0, 0, gg, LEVEL, t_work.thread_prefix_work_wl.gpu_wr_ptr(), num_items, t_work.thread_src_wl, pipe.in_wl(), pipe.out_wl());
              hipDeviceSynchronize();
            }
        }
      hipLaunchKernelGGL(bfs_kernel, dim3(blocks), dim3(__tb_bfs_kernel), 0, 0, gg, LEVEL, enable_lb, pipe.in_wl(), pipe.out_wl());
      hipDeviceSynchronize();
...
void gg_main_pipe_1_wrapper(CSRGraph& gg, int& LEVEL, PipeContextT<Worklist2>& pipe, dim3& blocks, dim3& threads)
{
  ...
  if (enable_lb)
    ...
    {
      int* cl_LEVEL;
      check_hip(hipMalloc(&cl_LEVEL, sizeof(int) * 1));
      check_hip(hipMemcpy(cl_LEVEL, &LEVEL, sizeof(int) * 1, hipMemcpyHostToDevice));
      bool * global_sense;
      bool* perSMsense;
      bool * done;
      unsigned int* global_count;
      ...
      // hipLaunchKernelGGL(gg_main_pipe_1_gpu, dim3(1), dim3(1), 0, 0, gg,LEVEL,pipe,blocks,threads,cl_LEVEL, enable_lb);
      //hipLaunchKernelGGL(gg_main_pipe_1_gpu_gb, dim3(gg_main_pipe_1_gpu_gb_blocks), dim3(__tb_gg_main_pipe_1_gpu_gb), 0, 0, gg,LEVEL,pipe,cl_LEVEL, enable_lb, gg_main_pipe_1_gpu_gb_barrier, global_sense, perSMsense, done, global_count, local_count, last_block, NUM_SM);
      hipLaunchKernelGGL(gg_main_pipe_1_gpu_gb, dim3(gg_main_pipe_1_gpu_gb_blocks), dim3(__tb_gg_main_pipe_1_gpu_gb), 0, 0, gg,LEVEL,pipe,cl_LEVEL, enable_lb, gg_main_pipe_1_gpu_gb_barrier, global_sense, perSMsense, done, global_count, local_count, last_block, NUM_SM, naive);
      hipEventRecord(stop);
      hipDeviceSynchronize();
      float ms;
      hipEventElapsedTime(&ms, start, stop);
      std::cout << " Kernel time (ms) " << ms << std::endl;
      check_hip(hipMemcpy(&LEVEL, cl_LEVEL, sizeof(int) * 1, hipMemcpyDeviceToHost));
      check_hip(hipFree(cl_LEVEL));
...
void gg_main(CSRGraph& hg, CSRGraph& gg)
{
  ...
  hipLaunchKernelGGL(bfs_init, dim3(blocks), dim3(threads), 0, 0, gg, start_node);
  hipDeviceSynchronize();
  int LEVEL = 1;
