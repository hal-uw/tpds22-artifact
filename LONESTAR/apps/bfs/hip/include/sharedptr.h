...
public:
  ...
  T *gpu_rd_ptr(int device = 1) /* device >= 1 */
  {
    ...
    if(ptrs[device] == NULL)
      HIP_SAFE_CALL(hipMalloc(&ptrs[device], nmemb * sizeof(T)));
  ...
  T *zero_gpu(int device = 1) { 
    ...
    HIP_SAFE_CALL(hipMemset(p, 0, sizeof(T) * nmemb));
    return p;
  }
  ...
  void copy(int src, int dst)
  {
    ...
    if(isCPU[dst] && !isCPU[src]) {
      HIP_SAFE_CALL(hipMemcpy(ptrs[dst], ptrs[src], nmemb * sizeof(T), hipMemcpyDeviceToHost));
    } else if (!isCPU[dst] && !isCPU[src]) {
      HIP_SAFE_CALL(hipMemcpy(ptrs[dst], ptrs[src], nmemb * sizeof(T), hipMemcpyDeviceToDevice)); 
    } else if (!isCPU[dst] && isCPU[src]) {
      HIP_SAFE_CALL(hipMemcpy(ptrs[dst], ptrs[src], nmemb * sizeof(T), hipMemcpyHostToDevice)); 
    } else
